...
void EllipticFilter::SynthesizeEllipticFilter()
{
    ...
    {
        WI.setParams(TermSpar2.ID, 0, ConnectionAux.at(i), 0);
        Wires.append(WI);
    }
}
...
void EllipticFilter::InsertEllipticSection(int &posx, int &Ni, QStringList &ConnectionAux, int j, bool flip, bool CentralSection)
{
    ...
    if (Specification.FilterType == Bandpass  && !Specification.isCLC)   Insert_Bandpass_2_Section(posx, Ni, ConnectionAux, j, flip, CentralSection);
    if (Specification.FilterType == Bandstop  && Specification.isCLC)    Insert_Bandstop_1_Section(posx, Ni, ConnectionAux, j, flip, CentralSection);
    if (Specification.FilterType == Bandstop  && !Specification.isCLC)   Insert_Bandstop_2_Section(posx, Ni, ConnectionAux, j, flip, CentralSection);
}
...
void EllipticFilter::Insert_Bandpass_1_Section(int &posx, int &Ni, QStringList &ConnectionAux, int j, bool flip, bool CentralSection)
{
    ...
    if (CentralSection)
    {
        ...
        //Shunt resonator
        Cshunt.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, vertical,
                         posx+50, 50, QString("%1").arg(NI.ID), "gnd");
        ...
        Lshunt.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, vertical,
                         posx, 50, QString("%1").arg(NI.ID), "gnd");
    ...
    if (!flip)
    {
        ...
        if (Cseries1_BP_T1 != 0)
        {
            Cseries1.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                               posx+75, -50, QString("N%1").arg(NumberComponents[ConnectionNodes]-1), QString("N%1").arg(NumberComponents[ConnectionNodes]-2));
            ...
            Lseries1.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                               posx+75, -100, QString("N%1").arg(NumberComponents[ConnectionNodes]-1), QString("N%1").arg(NumberComponents[ConnectionNodes]-2));
        ...
        Cseries2.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                           posx+40, 0, QString("NV%1").arg(virtual_nodes), QString("N%1").arg(NumberComponents[ConnectionNodes]-2));
    ...
    else
    {
        ...
        Cseries2.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                           posx+40, 0, QString("NV%1").arg(virtual_nodes), QString("N%1").arg(NumberComponents[ConnectionNodes]-1));
        Cseries2.val["C"] = num2str(Cseries2_BP_T1, Capacitance);
        Components.append(Cseries2);

        Lseries2.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                           posx+110, 0, QString("NV%1").arg(virtual_nodes++), QString("N%1").arg(NumberComponents[ConnectionNodes]));
        Lseries2.val["L"] = num2str(Lseries2_BP_T1, Inductance);
        Components.append(Lseries2);
    }
...
void EllipticFilter::Insert_Bandpass_2_Section(int &posx, int &Ni, QStringList &ConnectionAux, int j, bool flip, bool CentralSection)
{
    ...
    if (CentralSection)
    {
        ...
        //Series resonator
        Lseries.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                          posx+50, 0, QString("N%1").arg(NumberComponents[ConnectionNodes]), QString("NV%1").arg(++virtual_nodes));
        Lseries.val["L"] = num2str(Lseries_BP_T2, Inductance);
        Components.append(Lseries);

        Cseries.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                          posx-25, 0, QString("N%1").arg(NumberComponents[ConnectionNodes]+1), QString("NV%1").arg(virtual_nodes));
        Cseries.val["C"] = num2str(Cseries_BP_T2, Capacitance);
        Components.append(Cseries);

        WI.setParams(Lseries.ID, 0, Cseries.ID, 1);
        Wires.append(WI);

        WI.setParams(ConnectionAux.at(0), 0, Lseries.ID, 1);
        Wires.append(WI);
    ...
    if (Lseries_BP_T2 != 0)
    {
        if (!flip)
        {
            ...
            Lseries.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                              posx-0, 0, QString("N%1").arg(NumberComponents[ConnectionNodes]+1), QString("NV%1").arg(++virtual_nodes));
            Lseries.val["L"] = num2str(Lseries_BP_T2, Inductance);
            Components.append(Lseries);

            Cseries.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                              posx-75, 0, QString("NV%1").arg(virtual_nodes++), QString("N%1").arg(NumberComponents[ConnectionNodes]+2));
            Cseries.val["C"] = num2str(Cseries_BP_T2, Capacitance);
            Components.append(Cseries);
        ...
        {    Lseries.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                               posx-0, 0, QString("N%1").arg(NumberComponents[ConnectionNodes]), QString("NV%1").arg(++virtual_nodes));
            Lseries.val["L"] = num2str(Lseries_BP_T2, Inductance);
            Components.append(Lseries);

            Cseries.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                              posx-75, 0, QString("NV%1").arg(virtual_nodes++), QString("N%1").arg(NumberComponents[ConnectionNodes]+1));
            Cseries.val["C"] = num2str(Cseries_BP_T2, Capacitance);
            Components.append(Cseries);
        }
    }
    ...
    //Node
    NI.setParams(QString("N%1").arg(++NumberComponents[ConnectionNodes]), posx+75, 0);
    Nodes.append(NI);
    ...
    if (Lseries_BP_T2 != 0)
    {//If Lshunt -> 0 and Cshunt->+infty we should skip these components.
        Cshunt1.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, vertical,
                          posx+75, 50, QString("N%1").arg(NumberComponents[ConnectionNodes]), QString("NV%1").arg(virtual_nodes));
        Cshunt1.val["C"] = num2str(Cshunt1_BP_T2, Capacitance);

        Components.append(Cshunt1);

        Lshunt1.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, vertical,
                          posx+75, 100, QString("NV%1").arg(virtual_nodes), QString("NV%1").arg(virtual_nodes+1));
    ...
    if (Lseries_BP_T2 != 0)
    {
        Cshunt2.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, vertical,
                          posx+40, 150, QString("NV%1").arg(virtual_nodes), "gnd");
    ...
    {
        Cshunt2.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, vertical,
                          posx+40, 150, QString("N%1").arg(NumberComponents[ConnectionNodes]), "gnd");
        Cshunt2.val["C"] = num2str(Cshunt2_BP_T2, Capacitance);
        Components.append(Cshunt2);
    }
    //GND
    Ground.setParams(QString("GND%1").arg(++NumberComponents[GND]), GND, vertical, posx+40, 200, "", "");
    Components.append(Ground);
    ...
    if (Lseries_BP_T2 != 0)
    {
        Lshunt2.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, vertical,
    ...
    {
        Lshunt2.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, vertical,
                          posx+110, 150, QString("N%1").arg(NumberComponents[ConnectionNodes]), "gnd");
        Lshunt2.val["L"] = num2str(Lshunt2_BP_T2, Inductance);
        Components.append(Lshunt2);
    }

    //GND
    Ground.setParams(QString("GND%1").arg(++NumberComponents[GND]), GND, vertical, posx+110, 200, "", "");
    Components.append(Ground);
    ...
    if (((Specification.EllipticType == "Type B") || (Specification.EllipticType == "Type C")) && (j == Specification.order-1)) Ni--;

}


void EllipticFilter::Insert_Bandstop_2_Section(int &posx, int &Ni, QStringList &ConnectionAux, int j, bool flip, bool CentralSection)
{
    ComponentInfo Lshunt, Ground, Cseries1, Cseries2, Lseries1, Lseries2, Cshunt;
    NodeInfo NI;
    WireInfo WI;
...
void EllipticFilter::Insert_Bandstop_1_Section(int &posx, int &Ni, QStringList &ConnectionAux, int j, bool flip, bool CentralSection)
{
    ...
    if (CentralSection)
    {
        ...
        //Series resonator
        Cseries.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                          posx+25, -50, QString("N%1").arg(NumberComponents[ConnectionNodes]), QString("N%1").arg(NumberComponents[ConnectionNodes]+1));
        Cseries.val["C"] = num2str(Cseries_BS_T1, Capacitance);
        Components.append(Cseries);

        Lseries.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                          posx+25, 0, QString("N%1").arg(NumberComponents[ConnectionNodes]), QString("N%1").arg(NumberComponents[ConnectionNodes]+1));
        Lseries.val["L"] = num2str(Lseries_BS_T1, Inductance);
        Components.append(Lseries);

        WI.setParams(ConnectionAux.at(0), 0, Cseries.ID, 1);
        Wires.append(WI);
    ...
    if (Lseries_BS_T1 != 0)
    {
        ...
        else
        {
            //Series resonator
            Cseries.setParams(QString("C%1").arg(++NumberComponents[Capacitor]), Capacitor, horizontal,
                              posx-25, -50, QString("N%1").arg(NumberComponents[ConnectionNodes]+1), QString("N%1").arg(NumberComponents[ConnectionNodes]+2));
            Cseries.val["C"] = num2str(Cseries_BS_T1, Capacitance);
            Components.append(Cseries);

            Lseries.setParams(QString("L%1").arg(++NumberComponents[Inductor]), Inductor, horizontal,
                              posx-25, 0, QString("N%1").arg(NumberComponents[ConnectionNodes]+1), QString("N%1").arg(NumberComponents[ConnectionNodes]+2));
            Lseries.val["L"] = num2str(Lseries_BS_T1, Inductance);
            Components.append(Lseries);
        }
    }
    ...
    {//Cshunt -> +infty and Lshunt -> 0
        //GND
        Ground.setParams(QString("GND%1").arg(++NumberComponents[GND]), GND, vertical, posx+75, 200, "", "");
        Components.append(Ground);
    ...
    else
    {
        ...
        WI.setParams(NI.ID, 0, ConnectionAux.at(1), 0);
        Wires.append(WI);
        posx += 50;
