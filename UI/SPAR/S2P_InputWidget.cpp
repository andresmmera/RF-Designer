 ...
 *
 ***************************************************************************/
#include "S2P_InputWidget.h"

S2P_InputWidget::S2P_InputWidget() {
  ...
  SPAR_GridLayout->addWidget(Z0Label, 1, 0);
  SPAR_GridLayout->addWidget(Z0_SpinBox, 1, 1);
  SPAR_GridLayout->addWidget(Z0_Ohm_Label, 1, 2);

  // Add point
  AddPoint = new QPushButton("Add sample");
  SPAR_GridLayout->addWidget(AddPoint, 2, 0);
  ...
  SPAR_GridLayout->addWidget(S_Matrix_Label, 3, 0, 1, 3);
  ...
  SPAR_GridLayout->addWidget(S2PInputTable, 4, 0, 1, 3);
...
void S2P_InputWidget::UpdateS2PDataEntry() {
...
void S2P_InputWidget::LoadS2PFile() {
...
double S2P_InputWidget::getFreqScale(QString fscale) {
  double scale;
  if (fscale == "GHz")
    scale = 1e9;
  else if (fscale == "MHz")
...
void S2P_InputWidget::addSingleFreqData() {
...
// QTableWidget
void S2P_InputWidget::PutDataInTable() {
...
std::complex<double> S2P_InputWidget::ReadS2PFromUserInput(QString data) {
  QString R, I, M, A;
  double R_, I_, M_, A_;
  int sign = 1;
  int index;
...
void S2P_InputWidget::ClearTable() { S2PTable->clear(); }

QString S2P_InputWidget::rect2polar(std::complex<double> Z) {
