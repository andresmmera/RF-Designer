    ...
    : QWidget(parent)
{
    ...
    connect(QucsactorbrowseButton, SIGNAL(clicked()), this, SLOT(Qucsactorbrowse()));
...
void PreferencesDialog::Qucsactorbrowse()
{
    QFileDialog::getOpenFileName(this,
        tr("Path to qucsator"), "~", tr("Binary Files"));
}
...
{
  ToolSettings TS;
  TS.fstart = fstartLineedit->text().toDouble()*getScale(fstartScaleCombo->currentText());
  TS.fstop = fstopLineedit->text().toDouble()*getScale(fstopScaleCombo->currentText());
  ...
  TS.xstep = GraphFstepLineedit->text().toDouble()*getScale(XscaleStepCombobox->currentText());
...
void PreferencesDialog::S21ColorHandler()
{
   ...
   S21ColorButton->setStyleSheet(QString("background-color: %1").arg(TracesColor.at(0).name()));
}

void PreferencesDialog::S11ColorHandler()
{
   TracesColor.at(1) = QColorDialog::getColor(Qt::blue, this );
   S11ColorButton->setStyleSheet(QString("background-color: %1").arg(TracesColor.at(1).name()));
}

void PreferencesDialog::S22ColorHandler()
{
   TracesColor.at(2) = QColorDialog::getColor(Qt::darkGreen, this );
   S22ColorButton->setStyleSheet(QString("background-color: %1").arg(TracesColor.at(2).name()));
}
